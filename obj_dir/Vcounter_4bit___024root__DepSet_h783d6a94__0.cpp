...
VL_INLINE_OPT void Vcounter_4bit___024root___nba_sequent__TOP__0(Vcounter_4bit___024root* vlSelf) {
    ...
    if (vlSelf->reset_n) {
        if (vlSelf->en) {
            vlSelf->cnt = (0xfU & ((IData)(1U) + (IData)(vlSelf->cnt)));
        }
    } else {
        vlSelf->cnt = 0U;
    }
...
#endif
                    VL_FATAL_MT("counter_4bit.v", 1, "", "Active region did not converge.");
                }
...
#endif
                VL_FATAL_MT("counter_4bit.v", 1, "", "NBA region did not converge.");
            }
